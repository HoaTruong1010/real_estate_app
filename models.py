...
class Conversation(BaseModel):
    ...
    started_by = Column(Integer, ForeignKey('user.id', ondelete='SET NULL'))
    receive_by = Column(Integer, ForeignKey('user.id', ondelete='SET NULL'))
...
class Message(BaseModel):
    ...
    sent_from = db.Column(db.Integer, db.ForeignKey('user.id', ondelete='SET NULL'))
...
class User(BaseModel, UserMixin):
    ...
    posts = relationship('Posts', backref='user', lazy=False, cascade="all, delete",
        passive_deletes=True)

    reports = relationship('Reports', backref=backref("User", lazy=True), cascade="all, delete",
        passive_deletes=True)

    identifier = relationship('Identifier', backref=backref("User", lazy=True), cascade="all, delete",
        passive_deletes=True)
...
class Category(db.Model):
    ...
    posts = relationship('Posts', backref='category', lazy=False, cascade="all, delete",
        passive_deletes=True)
...
class Posts(BaseModel):
    ...
    user_id = Column(Integer, ForeignKey(User.id, ondelete='CASCADE'))
    category_id = Column(String(5), ForeignKey(Category.id, ondelete='CASCADE'), nullable=False)

    logs = relationship('Logs', backref=backref('posts', lazy=True, cascade="all, delete",
        passive_deletes=True))
    user_report = relationship('Reports', backref=backref('posts', lazy=True, cascade="all, delete",
        passive_deletes=True))
    images = relationship('Images', backref='posts', lazy=False, cascade="all, delete",
        passive_deletes=True)

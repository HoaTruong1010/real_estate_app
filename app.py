from houselandapp import app, mail, dao, login, oauth, es, index_name
from houselandapp import admin as ad
import math
    ...
    if request.method == 'POST':
        if 'cancel' in request.form:
            ...
            if user and not user.active:
                dao.delete_user(dao.encrypt_data(user.id))
        ...
        if 'login' in request.form:
            ...
            if user and user.active:
                if user.user_role == UserRoleEnum.WAITING_DELETE:
                    err_msg = dao.get_status_by_id(51)
                else:
                    dao.set_login(user)
...
def verify_phone():
    ...
    if request.method == "POST":
        if "phone" in request.form:
            ...
            if user.otp_code != otp_string:
                err_msg = dao.get_status_by_id(28)
            elif user and not user.active and user.otp_code == otp_string:
                try: 
...
def profile(user_id):
    ...
    if user:
        ...
        if user:
            posts = dao.load_posts_by_status_v2('profile',user_id=user.id)
    ...
    if request.method == 'POST' and "verify" in request.form:
        ...
        if user and user.otp_code == dao.hash_kw(otp_string) and value and type:
            try:
                ...
                print(error)
                err_msg = dao.get_status_by_id(9)
        elif user.otp_code != dao.hash_kw(otp_string):
            err_msg = dao.get_status_by_id(28)

    return render_template('verify_contact.html', err_msg=err_msg, id=id)
        ...
        if request.method == "POST":
            ...
            if "change-password" in request.form:
                ...
                if password and new_password and re_new_password:
                    ...
                    if pwd == user.password and check_password(new_password) and new_password==re_new_password:
                        try:
                            dao.update_password(user_id, new_password)
                            status=dao.get_status_by_id(15)
                        except Exception as error:
                            print(error)
                            status = dao.get_status_by_id(9)
            ...
            if "cancel" in request.form:
                try:
                    ...
                    ad.send_message(user.email, subject, msg_body, user.name)
                except Exception as error:
                    print(error)
                    status = dao.get_status_by_id(9)
            if "accept" in request.form:
                ...
                if full_name and gender.strip() in genders \
                    ...
                    try:
                        ...
                        ad.send_message(user.email, subject, msg_body, full_name)
                ...
                if restrict_from and restrict_to:                    
                    ...
                    if (end - start).total_seconds() < 0:
                        ...
                        try:
                            ...
                            print(err)
                            status = dao.get_status_by_id(9)
            if "delete-account-true" in request.form:
                msg = dao.request_delete_user(user.id)
                if msg == 1:
                    logout_user()
                    return redirect('/login_register')
                status = dao.get_status_by_id(9)
...
def report(post_id, content):
    try:
        ...
        status = dao.report(user_id=current_user.id, post_id=post_id, content=content)
        if status == 1:
            subject = "[Thông báo] Bài viết của bạn đã bị ẩn!"
            msg_body = f"Chúng tôi đã nhận nhiều báo cáo về bài viết \"{{post.title}}\".\n" \
...
def category_details(issales, cate_id):
    ...
    if q:
        must.append({"multi_match": {"query": q, "fields": ["description",'title', 'address']}})
    ...
    if results:     
        fisrt_result = results[0]
        results.sort(key = lambda post: math.sqrt(math.pow((post['location']['lat'] - fisrt_result['location']['lat']), 2) + math.pow((post['location']['lon'] - fisrt_result['location']['lon']), 2)))
        
    length = (len(results))
    if length >= 120:
        results = results[0:120]
    length = (len(results))
...
def details(post_id):
    ...
    if p and (p.status == PostsStatusEnum.ACCEPTED or p.user_id == current_user.id or current_user.user_role == UserRoleEnum.ADMIN):
        ...
        author_posts = [post for post in author_posts if post.id != int(post_id)]
        viewed_list = []
...
def api_check_properties():
    ...
    if post_id:
        post = dao.get_post_by_id(post_id)
    if not post or post.status in [PostsStatusEnum.WAITING, PostsStatusEnum.EDITED]:
        ...
        for item in compulsory:
            if 'address' in data and len(data.get('address').split(',')) != 3:
                return dao.get_status_by_id(32)
            if not item in data:
...
def edit_post(post_id):    
    ...
    if post and (current_user.id == post.user_id or current_user.user_role == UserRoleEnum.ADMIN):
        ...
        if request.method == "POST":
            ...
            if "submit" in request.form:
                ...
                try:
                    ...
                    post = dao.get_post_by_id(post_id)
                    if post.status not in [PostsStatusEnum.WAITING, PostsStatusEnum.EDITED]:
        ...
        print(category)               
        return render_template("edit.html", post=post, category=category,
                            address_arr=address_arr, status=status, expire=expire)
    else:
        abort(404)
...
def review(user_id):
    ...
    if user:
        ...
        for item in waiting_reviews:
            if (item.user_send.id == current_user.id and item.user_receive.id == user.id) or \
                (item.user_send.id == user.id and item.user_receive.id == current_user.id):
                flag = True
            ...
            last_review = dao.get_last_review(current_user.id, user.id)
            if last_review and last_review.review_at < item.updated_at and user.user_role == UserRoleEnum.PUBLISHER:
                accept_review = True
            elif not last_review and user.user_role == UserRoleEnum.PUBLISHER and flag:
                accept_review = True
